const fs = require('fs');
let code = fs.readFileSync('components/views/AgendaSection.tsx', 'utf8');

// Wait! AgendaSection.tsx was refactored into a re-export of agenda/index.tsx.
// My previous script blindly appended feelingUI to the end of a re-export file, breaking it.
// Let's restore AgendaSection.tsx to just be the re-export, and move the feelings UI to agenda/index.tsx.

fs.writeFileSync('components/views/AgendaSection.tsx', `/**
...
// Now process agenda/index.tsx
let agendaCode = fs.readFileSync('components/views/agenda/index.tsx', 'utf8');
...
if(!agendaCode.includes('import { useGamification }')) {
  ...
  // sometimes useModal isn't there, let's use a more robust fallback
  if(!agendaCode.includes('useGamification')) {
    agendaCode = agendaCode.replace(
...
fs.writeFileSync('components/views/agenda/index.tsx', agendaCode);
