...
const TrainerDashboard = ({ user, onLogout }: { user: any, onLogout: () => void }) => {
  ...
  const deleteClient = async (clientId: string) => {
    if (!confirm('¿Eliminar cliente permanentemente? Esta acción no se puede deshacer.')) return;
    try {
                          ...
                          <div key={client.id} className="flex items-center gap-4 p-4 hover:bg-gray-50 transition-colors">
                            <div className="w-12 h-12 rounded-full overflow-hidden bg-blue-100 flex items-center justify-center flex-shrink-0">
                              {client.photoURL ? (
                                ...
                                <span className="font-black text-blue-600 text-xl">{client.displayName?.[0]?.toUpperCase() || '?'}</span>
                              )}
                            </div>
                ...
                {showArchived && (
                  <div>
                    ...
                    {archivedClients.map(client => (
                      <div key={client.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-200 mb-3 opacity-70">
                        <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0">
                          <span className="font-black text-gray-400 text-lg">{client.displayName?.[0]?.toUpperCase() || '?'}</span>
                        </div>
                        <div className="flex-1 min-w-0">
