        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm p-4">
            <div className="container mx-auto px-4 max-w-6xl">
                <div className="flex items-center gap-4 mb-6">
                  <button onClick={onBack} className="flex items-center gap-1 text-blue-600 hover:underline text-sm font-bold bg-blue-50 px-3 py-1.5 rounded-lg transition-colors">
                    <ChevronLeft size={16} /> Clientes
                  </button>
                  <div className="flex-1 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                    <h2 className="text-2xl font-black text-gray-900">{client.displayName}</h2>
                    <span className="text-xs bg-blue-100 text-blue-700 px-3 py-1 rounded-full font-bold uppercase tracking-wider inline-block max-w-fit">{client.plan || 'Sin plan'}</span>
                  </div>
                  <button onClick={() => exportToCSV(client.id)} className="hidden sm:flex items-center gap-2 px-4 py-2 bg-green-600 text-white text-sm font-bold rounded-xl hover:bg-green-700 shadow-sm shadow-green-600/20 transition-all">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <div className="overflow-x-auto">
                            ...
                            {metrics.length === 0 && (
                                <div className="p-12 text-center text-gray-500">Sin datos de rendimiento aún.</div>
                            )}
                        </div>
                    </div>
                </div>
